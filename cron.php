...
while($row = $result->fetch_array()) {
	...
	while($line = fgets($fin)) {
		...
		if(substr($line, 0, 6) == "*cols:") {
			...
			$db->query("INSERT INTO bans (botid, server, name, ip, date, gamename, admin, reason, context, expiredate, openban_target, openban_id) VALUES ('{$config['botid']}', '$server', '$name', '$ip', NOW(), '', '', '$reason', 'ttr.cloud', DATE_ADD(NOW(), INTERVAL $duration MINUTES), '$target', '$id')");
		} else {
